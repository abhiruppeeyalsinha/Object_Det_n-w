// ════════════════════════════════════════════════════════════════════
// image_classical_detector.cpp (Updated with True Hybrid Fusion & Optimized Video Pipeline)
// ════════════════════════════════════════════════════════════════════
...
public:
    ...
    vector<BBox> detect_hybrid_all(const Mat& img) const
    {
        ...
        // Fuse masks and confidence maps seamlessly
        Mat combined_mask;
...
public:
    ...
    void run(const string& run_dir, string mode_str)
    {
        ...
        // Optimized MJPG Video Writer codec
        ...
        thread producer([this]() {
            ...
            while (cap_.read(temp))
            {
                ...
                // Skip every 2nd frame for real-time performance optimization
                if (idx % 2 != 0) continue;
