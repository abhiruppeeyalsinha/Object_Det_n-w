// ════════════════════════════════════════════════════════════════════
// image_classical_detector.cpp (Updated with Kalman Metrics in Reports)
// ════════════════════════════════════════════════════════════════════
...
// Extended Tracking Result Struct holding BBox + Kalman Internal States
...
public:
    ...
    {
        // 4 dynamic states: [x, y, dx, dy], 2 measurements: [x, y]
        kf.init(4, 2, 0);
...
void update_kalman_trackers(const vector<BBox>& detections, vector<TrackedTarget>& out_tracked_results)
{
    ...
    for (size_t t = 0; t < active_trackers.size(); ++t)
    {
        ...
        if (best_det_idx != -1)
        {
            ...
            out_tracked_results.push_back({
                ...
                active_trackers[t].get_vx(), 
                active_trackers[t].get_vy(), 
...
void process_frame_detections(Mat &frame, const BlobDetector &detector, DetMode det_mode, 
                              long long &elapsed_ms, int &det_count, vector<TrackedTarget> &out_tracked_results, bool is_video_stream)
{
    ...
    if (is_video_stream)
    {
        update_kalman_trackers(raw_detections, out_tracked_results);
    }
    else
    {
        out_tracked_results.clear();
    ...
    for (const auto &item : out_tracked_results)
    {
        ...
        string label = "ID " + to_string(id) + " (Vx:" + to_string(item.vx) + " Vy:" + to_string(item.vy) + ")";
...
public:
    ...
    void run(const string& run_dir, string mode_str)
    {
        ...
        report << "Mode: " << mode_str << " (Kalman Tracked with Internal Parameters)\n";
        report << "Frame\tID\tX\tY\tW\tH\tScore\tType\tArea\tAspect\textent\tDoG_Mean\tDist(px)\tVx\tVy\tError_Cov\n";
        report << "----------------------------------------------------------------------------------------------------------------------\n";
        mutex report_mutex;
        ...
        thread consumer([this, &writer, &report, &report_mutex]() {
            while (true)
            {
                ...
                process_frame_detections(item.frame, detector_, det_mode_, p_ms, d_count, tracked_results, true);
        ...
        report.close();
        cout << "\n[DONE] Kalman-tracked video processing complete. Report with states saved in: " << run_dir << "\n";
    }
...
int main(int argc, char **argv)
{
    ...
    if (!is_video)
    {
        ...
        process_frame_detections(sample_frame, detector, det_mode, processing_ms, det_count, tracked_results, false);
        ...
        report << "ID\tX\tY\tW\tH\tScore\tType\tArea\tAspect\textent\tDoG_Mean\tDist(px)\tVx\tVy\tError_Cov\n";
        report << "----------------------------------------------------------------------------------------------------------------------\n";
